    ...
    return (
        <>
            <div className={`flex flex-row md:justify-end text-${color} w-full font-rubik px-3 md:px-20 pt-7 pb-2 mb-2`}>
                <button onClick={() => setDropdown(!dropdown)} className="md:hidden flex bg-gradient-to-r from-redbg to-yellow-800 rounded-lg outline-white outline-none outline-1 w-8 h-8 self-center">
                    <Image src={'/menu.svg'} alt="" width={0} height={0} sizes="1px" className="h-auto w-full"/>
                </button>
                <div className={`${dropdown ? 'hidden' : 'block'} absolute flex flex-col divide-y-2 bg-white text-black rounded-xl p-3 translate-y-12 -translate-x-1 animate-dropdown`}>
